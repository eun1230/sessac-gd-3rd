...
/* 
useEffect의 의존성 배열에 함수가 들어가는 경우 useCallback hook을 사용하면 성능에 도움이 된다.  
...
//처음 컴포넌트 마운트 시 fetch데이터 함수 실행과 동시에 메모리에 콜백함수 자체가 저장된다.
//post아이디 값이 바뀌지 않는 다면 fetchData함수는 컴포넌트가 리렌더링되어도 다시 선언되지 않는다.
//계속 사용되지 않는 함수나 연산이 너무 복잡해서 계산하는데 많은 자원이 드는 함수는 useCallback으로 저장하면 된다.
*/